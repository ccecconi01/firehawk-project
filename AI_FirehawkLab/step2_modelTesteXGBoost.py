...
features = [
    ...
    'TEMPERATURA', 'HUMIDADERELATIVA', 'VENTOINTENSIDADE', 'VPD_kPa', # Weather and VPD
...
resultados.to_csv('resultados1_xgboost_limpo.csv', index=False)
print("-> 'resultados1_xgboost_limpo.csv' created (no floats and no negatives).")
...
if len(model.estimators_) > 0:
    ...
    plt.tight_layout()
    plt.savefig('gra1_importancia_xgboost.png')
...
if len(y_cols) > 0:
    ...
    plt.savefig('gra1_precisao_xgboost.png')