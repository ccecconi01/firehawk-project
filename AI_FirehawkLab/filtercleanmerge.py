...
# 2. REMOVE DUPLICATES 
# keep the first record found and delete ID repetitions
...
# 6.2 Remove negative or null durations (record errors)
df_final = df_final[df_final['Duracao_Horas'] > 0]
 
# 6.3 Remove burned area <= 0 (not useful for magnitude training)
if 'Area_Ardida_ha' in df_final.columns:
    df_final = df_final[df_final['Area_Ardida_ha'] > 0]
 
# 6.4 (Optional) keep only fires with operatives > 0 (i.e. actively fought)
if 'Operacionais_Man' in df_final.columns:
    df_final = df_final[df_final['Operacionais_Man'] > 0]
...
# Keep only columns that actually exist in the dataframe (avoid errors)
