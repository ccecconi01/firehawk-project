...
def refresh_data():
    """Manual trigger (non-blocking). Fires the pipeline in a background thread and
    returns immediately, so the HTTP request never blocks long enough to time out;
    the frontend polls the snapshot for the result. Shares the anti-overlap guard
    with the scheduler (a run already in progress is reported as 'busy')."""
    if _pipeline_lock.locked():
        return jsonify({"success": False, "status": "busy",
                        "error": "A refresh is already running."}), 409
    threading.Thread(
        target=lambda: run_pipeline_guarded('manual'),
        name='manual-refresh', daemon=True,
    ).start()
